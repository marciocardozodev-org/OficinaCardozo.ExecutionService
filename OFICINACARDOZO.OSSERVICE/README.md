...
# OficinaCardozo OS Service

Este microserviço implementa a gestão de Ordens de Serviço (OS) seguindo arquitetura em camadas, autenticação JWT, CI/CD, Docker e Kubernetes.
...
1. Clone o repositório:
	...
	cd OficinaCardozo.App/OFICINACARDOZO.OSSERVICE
	```
...
- Build da imagem:
  ```sh
  docker build -t oficinacardozo-osservice:latest .
  ```
- Rodar localmente:
  ```sh
  docker run -e JWT_KEY="sua-chave-secreta" -e ASPNETCORE_ENVIRONMENT=Development -p 8080:8080 oficinacardozo-osservice:latest
  ```

## Deploy Kubernetes
1. Crie o secret para a chave JWT:
	```sh
	kubectl create secret generic osservice-jwt-secret --from-literal=JWT_KEY="sua-chave-secreta" -n <namespace>
	```
2. Crie o ConfigMap para variáveis não sensíveis (ex: ambiente):
	```sh
	kubectl create configmap osservice-config --from-literal=ASPNETCORE_ENVIRONMENT=Production -n <namespace>
	```
